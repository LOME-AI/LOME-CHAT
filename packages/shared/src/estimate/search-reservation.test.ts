...
describe('WEB_SEARCH_RESERVATION_BASE_NANO_PER_MODEL', () => {
  it('is MAX_SEARCH_TOOL_CALLS × the per-call rate in nano-USD, pre-markup', () => {
    // Reproduces legacy worstCaseSearchCost()'s inner term (before applyFees).
...
describe('webSearchLineItem', () => {
  it('reserves the per-model base as a marked-up fixed item', () => {
    const item = webSearchLineItem(1);
    expect(item.label).toBe('web-search-reservation');
    expect(item.fixedNano).toBe(50_000_000n);
    expect(item.marksUp).toBe(true);
  });

  it('scales the reservation by the model count (legacy × modelCount)', () => {
    expect(webSearchLineItem(3).fixedNano).toBe(150_000_000n);
  });

...
| `minTurnCost`       | `fixedCosts + T × Σᵢ variableRate(mᵢ)`, where `T = maxᵢ (B(mᵢ, e_min) + MINIMUM_OUTPUT_TOKENS)`, at a **candidate payer's** tier — the turn priced at the **`eligible(m)` corner**: every fixed term its own definition names (input tokens, input storage, classifier reserve) plus the cheapest resolved reasoning budget and a minimum answer. The least this turn could possibly cost if that payer paid. **The token ceiling is solved ONCE across siblings and every sibling is priced at it — the widest corner — so the token term is `maxᵢ`, never a per-sibling `Σᵢ`.** A per-sibling sum is the intuitive reading and it **under-reserves**: worked from real catalog rates it yields 27,954,400 where the sufficient figure is 54,168,800, leaving a sibling ineligible at admission. The widest-corner figure is exactly sufficient and tight — one nano below it fails. **The payer decision consumes this, never a full estimate** — and it must be the whole corner: dropping the fixed terms or the reasoning term yields a number that is smaller but **not sufficient**, so a headroom that clears it would still fail admission. **`T × Σrates` is a bound, not a reserve:** it is what a decision may compare against, never a figure to charge or hold. |

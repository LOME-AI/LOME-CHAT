 ...
 * client input. Every conversations route is `session`-class, so the
 * authorizer guarantees a full principal before any handler runs; anything
 * else reaching this function is a composition defect (throw → 500), not an
 * expected error.
 */

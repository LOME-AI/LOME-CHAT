...
describe('CANONICAL_REASONING_EFFORTS', () => {
  ...
  it('rejects native upstream levels, the removed min rung, and non-levels', () => {
    for (const level of ['xhigh', 'minimal', 'min', 'off', 'auto', '']) {
      expect(CanonicalReasoningEffort.safeParse(level).success).toBe(false);
    }
  });
});

describe('REASONING_OFF', () => {
  it('is the single token for the reasoning-off rung', () => {
    expect(REASONING_OFF).toBe('off');
  });
});

describe('REASONING_EFFORT_SELECTIONS', () => {
  it('is auto, the canonical ladder, then off', () => {
  ...
  it('carries no second token for the reasoning-off rung', () => {
    expect(REASONING_EFFORT_SELECTIONS).not.toContain('none');
    expect(REASONING_EFFORT_SELECTIONS).not.toContain('min');
  });
...
describe('REASONING_EFFORT_LABELS', () => {
  ...
  it('labels the off selection as Min (founder-ruled copy 2026-07-23)', () => {
    expect(REASONING_EFFORT_LABELS[REASONING_OFF]).toBe('Min');
  });

  it('never labels a selection with its own id, so no surface can leak one', () => {
    expect(REASONING_EFFORT_LABELS.medium).not.toBe('medium');
    expect(REASONING_EFFORT_LABELS[REASONING_OFF]).not.toBe(REASONING_OFF);
  });

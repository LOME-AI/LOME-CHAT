/**
 * Magic-byte media validators for the real-provider integration suites.
 *
 * Hand-rolled magic-byte checks for the formats AI providers actually emit
 * (PNG, JPEG, WebP, MP4, WebM) — no new dependency. The single shared checker
 * both the image and video integration suites call, so the byte-signature and
 * size-bound logic exists once (CODE-RULES "One Implementation, Shared").
 * Adding a format means appending a signature constant and a case in
 * {@link detectMimeFromBytes}.
 */
...
/**
 * Assert `bytes` are within `sizeBoundsBytes` and carry a magic-byte signature
 * that resolves to one of `allowedMimeTypes`; return the detected mime. Throws
 * with a precise message on any violation so an integration failure names the
 * cause (too small / too large / undetectable / disallowed format).
 */

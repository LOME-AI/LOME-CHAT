...
/**
 * Pinned, deterministic AI model catalog for E2E.
 *
 * In E2E (`isE2E`) the mock AI client serves THIS array instead of fetching
 * the live public `/v1/models` endpoint, removing the only non-hermetic
 * external dependency from the E2E hot path. Production (real client) and plain
 * local dev (live fetch) are unaffected.
 *
 * Generated by running a real `/v1/models` snapshot through the SAME
 * production transform the live path uses (`publicModelEntrySchema` →
 * `toRawModel`), so the shape and pricing cannot diverge from prod. The
 * live-catalog-drift watchdog (`packages/shared/src/models/live-catalog-drift.test.ts`,
 * CI integration) independently asserts the live catalog still matches this
 * shape and the ZDR allow-list; when it flags drift, regenerate this snapshot.
 *
 * Do not hand-edit. Every entry is the verbatim `toRawModel` output for a real
 * catalog row; downstream `rawModelToModelInfo` / `processModels` /
 * `buildModelViewsForModality` consume it without throwing.
 */
export const E2E_MODEL_CATALOG: readonly RawModel[] = [
  {
  ...
  },
  {
    ...
    description: 'A version built for deep and general agentic thinking.',
    modality: 'text',
    context_length: 32_768,
    pricing: {
      prompt: '0',
  ...
  },
  {
    ...
    modality: 'text',
    context_length: 262_114,
    pricing: {
      prompt: '0.0000006',
      completion: '0.0000025',
    },
    supported_parameters: [],
    created: 1_762_387_200,
    architecture: {
      input_modalities: ['text'],
      output_modalities: ['text'],
    },

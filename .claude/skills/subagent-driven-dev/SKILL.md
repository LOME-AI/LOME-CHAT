...
1. **Open an unknowns log** and drive it empty.
   - Design decisions and anything irreversible → the human. Collect ALL questions and ask them in ONE AskUserQuestion round, not one interrupt per discovery. For fuzzy features, you may instead interview the human until a complete spec exists.
   - Factual unknowns → research subagents (Explore for the codebase, package-researcher for dependencies, deep-research for the web). Distilled findings only. A finding multiple briefs will need goes to `research/` as a file; cheap lookups stay in messages.
   - When many tasks face the same hard cross-file question, have research answer it ONCE as a `research/` artifact that briefs cite as authoritative over local guessing.
   - For a large feature with real design freedom, optionally spawn 2–3 architect agents with different mandates (minimal-change / clean-architecture / pragmatic), form your own view, and present trade-offs + a recommendation with the plan.

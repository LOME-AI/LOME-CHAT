...
/** In-flight node representation; the wire shape is `roadmapResponseSchema`. */
 ...
 * assigned). Exported so tests can hash the id and assert orphan routing.
 */
...
/**
 * Status precedence for the parent roll-up. Visually shipped (green) is
 * loudest, then in_progress, then planned. A parent's effective status is
 * the max of its own status and all descendants' statuses, so the board
 * never shows a parent in a "quieter" colour than one of its children.
 */
...
/**
 * Compute the 12-char hex SHA-256 prefix of a Linear id. Used as the opaque
 * id on the public wire so the marketing API never exposes raw Linear ids
 * (workspace-scoped UUIDs would leak ticket-numbering pace).
 */
...
/**
 * The nearest depth-1 ancestor of an issue (an ancestor that is itself a
 * top-level task). Linear allows arbitrary nesting; the board clamps
 * anything at depth ≥ 2 onto its depth-1 ancestor.
 */
export function findDepth1Ancestor(
  issueId: string,
...
/** Hash lookup for an id the same pipeline pass hashed — absence is a defect. */
export function opaqueIdFor(hashes: ReadonlyMap<string, string>, rawId: string): string {
  const hashed = hashes.get(rawId);
  if (hashed === undefined) throw new Error('roadmap normalize: missing id hash');
  return hashed;
}
...
function buildProjectNodes(
  ...
  for (const project of projects) {
    const hashedId = opaqueIdFor(projectHashes, project.id);
    nodes.push({
      id: hashedId,
      kind: 'project',
      parentId: null,
...
function buildIssueNodes(
  ...
  const nodes: NormalizedNode[] = [];
  for (const issue of filteredIssues) {
    const hashedId = opaqueIdFor(hashes.issues, issue.id);
...
/**
 * Walk the graph subtasks → tasks → projects, mutating each non-leaf
 * node's status to max(self, ...children). Subtasks are leaves (the
 * depth-1 flattening earlier in the pipeline means no deeper levels), so
 * the order is fixed and finite — no recursion needed. Tasks roll up first
 * so projects see post-rolled task statuses.
 */
...
/**
 * Derived progress per project node. `done` and `total` count top-level
 ...
 * card matches what the fraction measures.
 */
 ...
 *
 * 1. Filter issues to features and bugs only (the GraphQL query already
 *    filters defensively, but the mock fixture may contain other rows).
 * 2. Assign orphan issues (no Linear project) to the synthetic "Other"
 *    project.
 * 3. Build nodes for projects and issues with hashed opaque ids.

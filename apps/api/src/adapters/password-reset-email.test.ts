...
import { PASSWORD_CHANGED_EMAIL_SUBJECT } from './password-changed-email.js';
import {
...
describe('createPasswordResetEmailAdapter', () => {
  ...
  it('uses the fixed password-reset subject', async () => {
    ...
    expect(sender.getSentMessages()[0]?.subject).toBe(PASSWORD_RESET_EMAIL_SUBJECT);
  });

  it('uses a subject distinct from the password-changed notification', () => {
    expect(PASSWORD_RESET_EMAIL_SUBJECT).not.toBe(PASSWORD_CHANGED_EMAIL_SUBJECT);
  });

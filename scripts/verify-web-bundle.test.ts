...
describe('collectWebBundleViolations', () => {
  ...
  it('resolves the runtime from the installed package when no assets are supplied', async () => {
    const violations = await collectWebBundleViolations({ distributionDir });
...
describe('verifyWebBundle', () => {
  it('resolves for a compliant bundle', async () => {
    ...
    await expect(verifyWebBundle({ distributionDir, ortAssets: assets })).resolves.toBeUndefined();
  });
  ...
  it('throws naming every violation it found', async () => {
    ...
    await expect(verifyWebBundle({ distributionDir, ortAssets: assets })).rejects.toThrow(
      /ort-wasm-simd-threaded\.jsep-B0T3yYHD\.wasm[\S\s]*tts\.worker-abc\.js/
    );
  });

...
import {
  ...
  createAppPasswordChangedEmailPort,
  createPasswordChangedEmailAdapter,
} from './password-changed-email.js';
...
describe('createPasswordChangedEmailAdapter', () => {
  function harness(sender: EmailSender): {
    port: ReturnType<typeof createPasswordChangedEmailAdapter>;
    warns: RecordedWarn[];
    resolveCount: () => number;
  } {
    const { telemetry, warns } = recordingTelemetry();
    let calls = 0;
    const port = createPasswordChangedEmailAdapter(() => {
      calls += 1;
  ...
  it('sends to the given address', async () => {
    ...
    const result = await port.sendPasswordChangedEmail({ to: 'user@example.com' });
    expect(result.isOk()).toBe(true);
    expect(sender.getSentMessages()[0]?.to).toBe('user@example.com');
  });

  it('uses the fixed password-changed subject', async () => {
    const sender = createMockEmailSender();
    const { port } = harness(sender);
    await port.sendPasswordChangedEmail({ to: 'user@example.com' });
    expect(sender.getSentMessages()[0]?.subject).toBe(PASSWORD_CHANGED_EMAIL_SUBJECT);
  });

  it('greets by user name when one is provided', async () => {
    const sender = createMockEmailSender();
    const { port } = harness(sender);
    await port.sendPasswordChangedEmail({ to: 'user@example.com', userName: 'Sam' });
  ...
  it('logs the failure error code through the typed logger', async () => {
    ...
    expect(warns).toEqual([
      { msg: 'password-changed email send failed', fields: { errorCode: 'unavailable' } },
    ]);
  });
  ...
  it('resolves its dependencies freshly on every send', async () => {
    ...
    await port.sendPasswordChangedEmail({ to: 'a@example.com' });
    await port.sendPasswordChangedEmail({ to: 'b@example.com' });
    expect(resolveCount()).toBe(2);
  });
});

describe('createAppPasswordChangedEmailPort', () => {
  const DATABASE_URL = process.env['DATABASE_URL'];
  if (DATABASE_URL === undefined || DATABASE_URL === '') {
    throw new Error('DATABASE_URL is required for the app password-changed email port tests');
  }
  ...
  it('sends through the env-selected sender inside a request context', async () => {
    ...
    app.post('/send', async (c) => {
      ...
      const port = createAppPasswordChangedEmailPort();
      const result = await port.sendPasswordChangedEmail({ to: 'user@example.com' });

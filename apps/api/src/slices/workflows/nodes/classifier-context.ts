/**
 * Conversation-text truncation for the classifier call.
 *
 * This handles message content, so it lives with the caller that holds the
 * content rather than in the shared money layer — that layer is content-free by
 * contract (counts, rates and identifiers only). The budget it fills,
 * {@link MAX_CLASSIFIER_CONTEXT_CHARS}, stays there because the classifier
 * reserve prices that cap rather than the realized text.
 */

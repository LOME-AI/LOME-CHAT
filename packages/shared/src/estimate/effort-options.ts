...
/**
 * One entry of the turn's real choice set: a canonical rung or `'none'`
 * (displayed as Min — reasoning off). `auto` is a selection, not a choice —
 * it never appears here; the menu prepends it and the classifier enumerates
 ...
 * `'none'` sits below every rung, so the same nearest-below walk covers Min.
 */
export function resolveEffortForModel(
  ...
  const chosenPosition = chosen === 'none' ? -1 : ladderPosition(chosen);
...
export function turnEffortOptions(models: readonly ReasoningPlanModel[]): EffortOption[] {
  ...
  const choices: EffortChoice[] = [
    ...(models.some((model) => canDisableReasoning(model)) ? (['none'] as const) : []),
